...
def certificate_print(
    ...
    # 5) الكلية (استنتاج من أهداف الدورة أو من بيانات المستخدم)
    ...
    # ثانياً: لو لم يتم العثور، جرب استخدام كلية المستخدم (hod_college) كما كان سابقًا
...
def skills_record_report(
    ...
    # بناء قائمة الكليات الفريدة لعرض التواقيع إن وُجدت
    colleges_map = {}
    for c in trainee_info.get("courses", []):
        col = c.get("college")
        if col and col.get("name"):
